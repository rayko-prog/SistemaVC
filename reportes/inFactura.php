...
function addClient( $ref )
{
	...
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+3 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,5, "PROVEEDOR", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1 + 9 );
	$this->SetFont( "Arial", "", 10);
...
// Client address
function addClientAdresse( $proveedor,$domicilio,$num_documento,$email,$telefono )
{
	$r1     = $this->w - 180;
	$r2     = $r1 + 68;
	$y1     = 40;
	...
	$this->SetXY( $r1, $y1+5);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 150, 4, $proveedor);
	$this->SetXY( $r1, $y1+10);
...
function addLineFormat( $tab )
{
	...
	foreach ( (array) $colonnes as $lib => $pos )
	{
		if ( isset( $tab["$lib"] ) )
			$format[ $lib ] = $tab["$lib"];
	}
